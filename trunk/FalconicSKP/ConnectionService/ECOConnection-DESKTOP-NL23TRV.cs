...
//                _actual_customer._bRecharge = false;
                ...
                if (ConnectionService.ClientConnection.ParseCustomerData(frame, this.Name, ref card))
                {
                    ...
                    if (card.CustomerNumber != 0)
                    {
                        ...
                        HasCardAccessToLocationFractionByCustomerNumber checkAccess = new HasCardAccessToLocationFractionByCustomerNumber((int)card.CustomerNumber, this._operator_id, this._location_id);
                    ...
                    else
                    {
                        ...
                        HasCardAccessToLocationFractionByCardUuid checkAccess = new HasCardAccessToLocationFractionByCardUuid(uuid, this._operator_id, this._location_id);
        ...
        private void process_message(string frame)
        {
            try
            {
                ...
                if (toks.GetLength(0) == 4)
                {
                    ...
                    {
                        string text = "";
                        SKP.AlertingUser[] alerting_users = new SKP.AlertingUser[256];
                        int number_of_users = 0;

                        for (int i = 0; i < 256; i++)
                        {
                            alerting_users[i] = new SKP.AlertingUser();
                        }

//                        ConnectionControl.SkpApiClient.GetTranslation(language_keyword, this._location_language_code);
            ...
            try
            {
                ...
                for (int i = 0; i < numberOfExpectedEntries; i++)
                {
                    try
                    {
                        ...
                        uint customerNumber = Convert.ToUInt32(tranCustomerNumber);
                        ...
                        if (!bAlphanumericCustomerNumber && customerNumber == 0)
                        {
                            try
                            {
                                StatusMessageDto statusMessage = new StatusMessageDto(transStatusId, 0.0F, false, false, transDate);
                        ...
                        if (bAlphanumericCustomerNumber)
                        {
                            try
                            {
                                CreateInsertionTransactionByCardUuid trans = new CreateInsertionTransactionByCardUuid(alibiStoreNumber, cardSerialNumber, transAmount, this._container_id, transCurrentBalance,
                        ...
                        else
                        {
                            try
                            { 
                                CreateInsertionTransactionByCustomerNumber trans = new CreateInsertionTransactionByCustomerNumber(alibiStoreNumber, cardSerialNumber, transAmount, this._container_id,
                                    transCurrentBalance, Convert.ToInt32(tranCustomerNumber), transDuration, transLocationId, null, this._operator_id, transStatusId,
        ...
        /// <returns></returns>
        private bool AnalyzeCustomerData(string str, ref SKP.Customer customer)
        {
            try
            {
                string[] toks = str.Split(new char[] { '=', ',' });
...
//                LogFile.WriteMessageToLogFile("Analyze customer data with string: {0}", str);
                ...
                if (toks.GetLength(0) >= 12)
                {
                    try
                    {
                        ...
                        SKP.eCARD_TYPE card_type_id = SKP.eCARD_TYPE.INVOICE;
                        int customer_id = -1;
                ...
                if (toks.GetLength(0) >= 16)
                {
                    try
                    {
                        ...
                        try
                        {
                            ...
                            LogFile.WriteMessageToLogFile("{0} Location: {1}, Language: {2}, nightlock active: {3}", this.Name, _locationParams.Name, _locationParams.Language.CalculatedLanguageCode, _locationParams.NightLockActive ? "true" : "false");
