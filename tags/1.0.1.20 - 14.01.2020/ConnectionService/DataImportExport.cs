...
//                    txtLine += customer.LocationGroup + ";";
                ...
                if (_export_format.ToLower().IndexOf(".xml") != -1)
                {
                    ...
                    string file_name_server = String.Format("Ents/{0}/Export/{1}", _operator_id, fi.Name);
        ...
        private bool ftp_upload_file(string local_file_name, string server_file_name, string ftp_server, string username, string password)
        {
            ...
            try
            {
                using (System.IO.FileStream fs = new System.IO.FileStream(local_file_name, System.IO.FileMode.Open))
                {
            ...
            {                
                ...
                using (System.IO.Stream stm = request.GetRequestStream())
                {
                    ...
                    stm.Write(fileContents, 0, fileContents.Length);
                    stm.Close();
                }
        ...
        private bool ftp_download_file_if_present()
        {
            ...
            try
            {
                ...
                try
                {
                    // create the ftp request
                    WebRequest request = FtpWebRequest.Create(uri);
                    request.Credentials = new NetworkCredential(_params.FtpUser, _params.FtpPassword);
                ...
                {
                    string uri_file = string.Format("ftp://{0}/Ents/{1}/Import/{2}", _params.FtpServer, _operator_id, (string)directory[i]);
                    ...
                    try
                    {
                        ...
                        request1.Credentials = new NetworkCredential(_params.FtpUser, _params.FtpPassword);

                        byte[] file_data = request1.DownloadData(uri_file);

                        FileStream file = new FileStream(output_filename, FileMode.Create);

                        file.Write(file_data, 0, file_data.GetLength(0));
                        file.Close();
                    }
                    catch (Exception excp)
                    {
                        LogFile.WriteErrorToLogFile("{0}: Exception {1} occured while trying to dowload file {2} on server!", _params.Operator,
                            excp.Message, uri_file);
                    ...
                    try
                    {
                        ...
                        request2.Credentials = new NetworkCredential(_params.FtpUser, _params.FtpPassword);
                        ...
                        string server_file_name = String.Format("Ents/Sicherung/{0}_{1}_{2}_{3}_{4}", _operator_id, dateString,
                            DateTime.Now.Hour, DateTime.Now.Minute, directory[i]);

                        // upload with advanced ftp user
                        if (ftp_upload_file(output_filename, server_file_name, _params.FtpServer, "erleand", "Erler963"))
                        {
